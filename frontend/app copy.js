...
async function buildProductionTab() {
    ...
    try {
        // Get all unique components from analyzed data
        const allComponents = [];
        const seen = new Set();
        ...
        analysisData.all_stories.forEach(story => {
            if (story.components) {
                story.components.forEach(comp => {
                    const key = `${comp.type}.${comp.api_name}`;
                    if (!seen.has(key)) {
                        seen.add(key);
                        allComponents.push({
                            name: comp.api_name,
        ...
        container.innerHTML = `
            <div style="padding: 40px; text-align: center;">
                ...
                <p style="color: #666; margin-top: 20px;">Make sure Flask is running on port 5000</p>
            </div>
        `;
    }
}

function displayProductionState(data) {
    const container = document.getElementById('tab-production');
