...
document.addEventListener('DOMContentLoaded', () => {
 ...
 window.handleGeneratePlanClick = function (e) {
   try {
     ...
     saveDeploymentPlan(plan);
     renderDeploymentPlan(plan);
    // Take the user to the plan they just created
     if (typeof showTab === 'function') {
       showTab('deploy');
     } else {
       const pane = document.getElementById('tab-deploy');
       if (pane) {
         // fallback show if showTab is unavailable here
         document.querySelectorAll('.tab-content').forEach(n => n.style.display = 'none');
...
if (!el) return; // ultimate guard
  ...
  el.innerHTML = `
    <div class="plan-header">
      ...
      <div><strong>Summary:</strong> total=${plan.summary.total}, approved=${plan.summary.approved}, excluded=${plan.summary.excluded}</div>
      <div class="actions">
...
ensureDeployTab();
   ...
   renderDeploymentPlan(plan);
  try { if (typeof showTab === 'function') showTab('deploy'); } catch {}
 }
...
// Ensure the Deployment Plan tab exists and is visible (no-op if already present)
function ensureDeployTab() {
  // Make sure the DevOps view is visible
  try { if (typeof switchView === 'function') switchView('devops'); } catch {}

  // Create the tab pane if missing
  let pane = document.getElementById('tab-deploy');
  if (!pane) {
    const devops = document.getElementById('devopsView') || document.body;
    pane = document.createElement('div');
    pane.id = 'tab-deploy';
    pane.className = 'tab-content';
    pane.style.display = 'none';
    devops.appendChild(pane);
  }
  return pane;
}
