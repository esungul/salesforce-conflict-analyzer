...
class Config:
    ...
    VALIDATION_USE_EVIDENCE: bool = True
    VALIDATION_SKIP_LARGE_FILES: bool = True
    VALIDATION_SKIP_UNAVAILABLE: bool = True
    VALIDATION_REQUIRE_CRITICALITY: bool = True
    
    # Thresholds
    VALIDATION_LARGE_FILE_MB: int = 10
    VALIDATION_MAX_FILES: int = 500
    VALIDATION_MAX_TIME_SEC: int = 30
    
    # Default validation levels
    VALIDATION_LEVEL_DEFAULT: str = "standard"
    VALIDATION_LEVEL_CRITICAL: str = "full"
    VALIDATION_LEVEL_HOTFIX: str = "standard"
    VALIDATION_LEVEL_FIRST_DEPLOY: str = "standard"
    
    # Unavailable objects (comma-separated list as string)
    VALIDATION_UNAVAILABLE_OBJECTS: str = ""
...
def get_config() -> Config:
    ...
    _cfg = Config(
        ...
        COMPONENT_TYPES_YAML=os.getenv("COMPONENT_TYPES_YAML"),
        
        # ========== NEW: Validation Configuration ==========
        VALIDATION_ENABLED=_get_bool("VALIDATION_ENABLED", True),
        VALIDATION_USE_EVIDENCE=_get_bool("VALIDATION_USE_EVIDENCE", True),
        VALIDATION_SKIP_LARGE_FILES=_get_bool("VALIDATION_SKIP_LARGE_FILES", True),
        VALIDATION_SKIP_UNAVAILABLE=_get_bool("VALIDATION_SKIP_UNAVAILABLE", True),
        VALIDATION_REQUIRE_CRITICALITY=_get_bool("VALIDATION_REQUIRE_CRITICALITY", True),
        
        # Thresholds
        VALIDATION_LARGE_FILE_MB=_get_int("VALIDATION_LARGE_FILE_MB", 10),
        VALIDATION_MAX_FILES=_get_int("VALIDATION_MAX_FILES", 500),
        VALIDATION_MAX_TIME_SEC=_get_int("VALIDATION_MAX_TIME_SEC", 30),
        ...
        VALIDATION_LEVEL_HOTFIX=os.getenv("VALIDATION_LEVEL_HOTFIX", "standard"),
        VALIDATION_LEVEL_FIRST_DEPLOY=os.getenv("VALIDATION_LEVEL_FIRST_DEPLOY", "standard"),
        
        # Unavailable objects
        VALIDATION_UNAVAILABLE_OBJECTS=os.getenv("VALIDATION_UNAVAILABLE_OBJECTS", ""),
    )
    return _cfg