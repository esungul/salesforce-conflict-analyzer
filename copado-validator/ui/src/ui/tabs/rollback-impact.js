...
// Now shows BOTH rollback options (Previous Owner + Production) where possible,
// and marks the default recommendation.
...
function buildRollbackPlanForStory(story, analysis) {
  ...
  for (var i = 0; i < comps.length; i++) {
    ...
    // Find previous owner (next most recent story that is not the current one)
    var prevOwner = null;
    if (usage && usage.length) {
      ...
      for (var j = 0; j < sorted.length; j++) {
        if (sorted[j].story_id !== sId) { prevOwner = sorted[j]; break; }
      }
    }

    // Production option (if baseline exists on component)
    var hasProd = !!c.production_commit_date;
    var prodOpt = hasProd ? {
      mode: 'prod',
      production_story_id: c.production_story_id,
      production_commit_date: c.production_commit_date
    } : null;

    var prevOpt = prevOwner ? {
      mode: 'previous_story',
      story_id: prevOwner.story_id,
      commit_date: prevOwner.commit_date
    } : null;

    // Recommendation hierarchy: prefer previous owner if exists, else Production
    var defaultChoice = prevOpt ? 'previous_story' : (prodOpt ? 'prod' : null);
    var postOwnerForDefault =
      defaultChoice === 'previous_story' ? (prevOwner && prevOwner.story_id) :
      defaultChoice === 'prod' ? (c.production_story_id || 'production') :
      null;
    ...
    plan.push({
      ...
      // NEW: both options presented
      rollback_options: {
        previous_owner: prevOpt,
        production: prodOpt
      },
      default_choice: defaultChoice, // 'previous_story' | 'prod' | null
      post_state_owner: postOwnerForDefault
    });
  }
...
function renderStoryPlanHTML(plan, analysis) {
  ...
  for (var i = 0; i < plan.plan.length; i++) {
    ...
    // Build BOTH options where available
    var optLines = [];
    // Previous owner option
    if (p.rollback_options && p.rollback_options.previous_owner) {
      var po = p.rollback_options.previous_owner;
      var recommendedBadge = (p.default_choice === 'previous_story') ? ' <span class="rb-badge-rec">Recommended</span>' : '';
      optLines.push(
        '<div class="rb-opt">' +
          '<span class="rb-opt-title">Previous owner</span>: ' +
          '<strong>' + escapeHtml(po.story_id) + '</strong>' +
          (po.commit_date ? ' <span class="muted">(' + fmtDate(po.commit_date) + ')</span>' : '') +
          recommendedBadge +
        '</div>'
      );
    }
    // Production option
    if (p.rollback_options && p.rollback_options.production) {
      var pr = p.rollback_options.production;
      var recommendedBadge2 = (p.default_choice === 'prod') ? ' <span class="rb-badge-rec">Recommended</span>' : '';
      optLines.push(
        '<div class="rb-opt">' +
          '<span class="rb-opt-title">Production</span>' +
          (pr.production_commit_date ? ': <span class="muted">' + fmtDate(pr.production_commit_date) + '</span>' : '') +
          recommendedBadge2 +
        '</div>'
      );
    }
    if (optLines.length === 0) {
      optLines.push('<div class="muted">—</div>');
    }

    // Post-state owner (for the default choice)
    var postOwner = p.post_state_owner ? escapeHtml(p.post_state_owner) : '—';
    ...
    html += '<tr class="rb-actions-row">' +
      ...
      '<td>' + optLines.join('') + '</td>' +
      '<td style="font-weight:500;">' + postOwner + '</td>' +
    '</tr>';
  }
...
function injectRollbackStyles() {
  ...
  var css = '\n' +
    ...
    '  .rb-badge-rec { display:inline-block; margin-left:8px; padding:2px 6px; font-size:11px; border:1px solid #2e7d32; color:#1b5e20; background:#e8f5e9; border-radius:10px; font-weight:700; text-transform:uppercase; letter-spacing:0.4px; }\n' +
    '  .rb-opt { margin-bottom:6px; }\n' +
    '  .rb-opt-title { font-weight:600; color:#101828; }\n' +
    '  /* Modal */\n' +
